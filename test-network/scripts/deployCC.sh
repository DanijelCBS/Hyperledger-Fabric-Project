...
# iznad komentara posle obrisati
...
installChaincode() {
  ORG=$1
  PEER_NO=$2
  setGlobals $ORG $PEER_NO
  set -x
  peer lifecycle chaincode install ${CC_NAME}.tar.gz >&log.txt
  res=$?
  { set +x; } 2>/dev/null
  cat log.txt
  verifyResult $res "Chaincode installation on peer${PEER_NO}.org${ORG} has failed"
  successln "Chaincode is installed on peer${PEER_NO}.org${ORG}"
}
...
queryInstalled() {
  ...
  verifyResult $res "Query installed on peer${PEER_NO}.org${ORG} has failed"
  successln "Query installed successful on peer${PEER_NO}.org${ORG} on channel"
}
...
approveForMyOrg() {
  ...
  { set +x; } 2>/dev/null
  cat log.txt
  verifyResult $res "Chaincode definition approved on peer${PEER_NO}.org${ORG} on channel '$CHANNEL_NAME' failed"
  successln "Chaincode definition approved on peer0.org${PEER_NO} on channel '$CHANNEL_NAME'"
}
...
checkCommitReadiness() {
  ...
  shift 1
  setGlobals $ORG $PEER_NO
  infoln "Checking the commit readiness of the chaincode definition on peer${PEER_NO}.org${ORG} on channel '$CHANNEL_NAME'..."
  local rc=1
  local COUNTER=1
  # continue to poll
  ...
  while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ]; do
    sleep $DELAY
    infoln "Attempting to check the commit readiness of the chaincode definition on peer${PEER_NO}.org${ORG}, Retry after $DELAY seconds."
    set -x
  ...
  if test $rc -eq 0; then
    infoln "Checking the commit readiness of the chaincode definition successful on peer${PEER_NO}.org${ORG} on channel '$CHANNEL_NAME'"
  else
    fatalln "After $MAX_RETRY attempts, Check commit readiness result on peer${PEER_NO}.org${ORG} is INVALID!"
  fi
}
...
queryCommitted() {
  ...
  infoln "Querying chaincode definition on peer${PEER_NO}.org${ORG} on channel '$CHANNEL_NAME'..."
  local rc=1
  local COUNTER=1
  # continue to poll
  # we either get a successful response, or reach MAX RETRY
  while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ]; do
    sleep $DELAY
    infoln "Attempting to Query committed status on peer${PEER_NO}.org${ORG}, Retry after $DELAY seconds."
    set -x
  ...
  cat log.txt
  if test $rc -eq 0; then
    successln "Query chaincode definition successful on peer${PEER_NO}.org${ORG} on channel '$CHANNEL_NAME'"
  else
    fatalln "After $MAX_RETRY attempts, Query chaincode definition result on peer${PEER_NO}.org${ORG} is INVALID!"
  fi
}
...
successln "Starting commitChaincodeDefinition"
## now that we know for sure both orgs have approved, commit the definition
commitChaincodeDefinition 1 0 2 0 3 0
successln "Finished commitChaincodeDefinition"
